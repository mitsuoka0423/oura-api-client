 ...
 * no description
 */
export class WorkoutsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param nextToken 
     */
    public async workoutsRouteWorkoutGet(startDate?: string, endDate?: string, nextToken?: string, _options?: Configuration): Promise<RequestContext> {
        ...
        // Path Params
        const localVarPath = '/v2/usercollection/workout';
...
export class WorkoutsApiResponseProcessor {

    /**
     * Unwraps the actual response sent by the server from the response context and deserializes the response content
     * to the expected objects
     *
     * @params response Response returned by the server for a request to workoutsRouteWorkoutGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async workoutsRouteWorkoutGet(response: ResponseContext): Promise<WorkoutResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: WorkoutResponse = ObjectSerializer.deserialize(
