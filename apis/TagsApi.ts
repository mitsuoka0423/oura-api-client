...
export class TagsApiRequestFactory extends BaseAPIRequestFactory {
     ...
     * @param nextToken 
     */
    public async tagsRouteTagGet(startDate?: string, endDate?: string, nextToken?: string, _options?: Configuration): Promise<RequestContext> {
        ...
        // Path Params
        const localVarPath = '/v2/usercollection/tag';
...
export class TagsApiResponseProcessor {
     ...
     *
     * @params response Response returned by the server for a request to tagsRouteTagGet
     * @throws ApiException if the response code was not in [200, 299]
     */
     public async tagsRouteTagGet(response: ResponseContext): Promise<TagResponse > {
        const contentType = ObjectSerializer.normalizeMediaType(response.headers["content-type"]);
        if (isCodeInRange("200", response.httpStatusCode)) {
            const body: TagResponse = ObjectSerializer.deserialize(
